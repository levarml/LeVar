...
trait Database {
  ...
  /**
    ...
    */
  def listData(org: String,
               datasetId: String,
               after: Option[String] = None,
               max: Option[Int] = None,
               withValue: Boolean): ResultSet[Datum]
  ...
  /**
    ...
    */
  def getExperiment(
      org: String, datasetId: String, experimentId: String): Experiment
  ...
  /**
    ...
    */
  def updateExperiment(org: String,
                       datasetId: String,
                       experimentId: String,
                       updates: Experiment.Update)
...
object impl extends Database with JsonLogging {
  ...
  def delAuth(user: String) {
    ...
    if (deletedUsers.isEmpty) {
      ...
      throw new UnexpectedResultException(
          s"unexpected users deleted matching $user: ${deletedUsers.mkString(", ")}")
    } else {
  ...
  def renameAuth(current: String, next: String) {
    ...
    if (updatedUsers.isEmpty) {
      ...
      val msg =
        s"unexpected users updated matching $current: ${updatedUsers.mkString(", ")}"
  ...
  def resetPassword(user: String, currentPass: String, nextPass: String) {
    ...
    val updatedUsers = DB.localTx { implicit setssion =>
      sql"""update auth
            ...
            returning username""".map(_.string("username")).list.apply()
    }
    if (updatedUsers.isEmpty) {
      throw new UnauthorizedException(user)
    } else if (updatedUsers != List(user)) {
      val msg =
        s"unexpected users updated matching $user: ${updatedUsers.mkString(", ")}"
      throw new UnexpectedResultException(msg)
    } else {
  ...
  def addOrg(id: String) {
    ...
    if (newIds.isEmpty) {
      ...
      val msg =
        s"unexpected organizations created matching $id: ${newIds.mkString(", ")}"
      throw new UnexpectedResultException(msg)
    } else {
  ...
  def delOrg(id: String) {
    ...
    if (deletedOrgs.isEmpty) {
      ...
      val msg =
        s"unexpected orgs deleted matching $id: ${deletedOrgs.mkString(", ")}"
      throw new UnexpectedResultException(msg)
    } else {
  ...
  private def lookupUserIds(users: Seq[String]): Seq[String] = DB.readOnly {
    implicit session =>
      val usersUniq = users.distinct
      val userNameIds =
        sql"select username, auth_id::text from auth where username in ($usersUniq)".map {
          r =>
            (r.string("username"), r.string("auth_id"))
        }.list.apply()
  ...
  def addToOrg(id: String, users: Seq[String]) {
    ...
    DB.localTx { implicit session =>
      ...
      val orgIdLookup =
        sql"select org_id::text from org where provided_id = $id"
          .map(_.string("org_id"))
          .single
          .apply()
  ...
  def listUserOrgs(user: String): Seq[String] = DB.readOnly {
    implicit session =>
      sql"""select org.provided_id org
            ...
            where auth.username = $user""".map(_.string("org")).list.apply()
  }

  def listOrgs: Seq[String] = DB.readOnly { implicit session =>
    sql"select provided_id from org".map(_.string("provided_id")).list.apply()
  }

  def listOrgUsers(org: String): Seq[String] = DB.readOnly {
    implicit session =>
      sql"""select username
          from auth
          inner join org_membership on auth.auth_id = org_membership.auth_id
          inner join org on org_membership.org_id = org.org_id
          where org.provided_id = $org""".map(_.string("username")).list.apply()
  }
  ...
  def renameOrg(current: String, next: String) {
    ...
    val updatedOrgs = DB.localTx { implicit session =>
      sql"""update org
            ...
            returning provided_id""".map(_.string("provided_id")).list.apply()
    }
    ...
    if (updatedOrgs.isEmpty) {
      ...
      val msg =
        s"unexpected orgs updated matching $current: ${updatedOrgs.mkString(", ")}"
      throw new UnexpectedResultException(msg)
    } else {
      info("status" -> "success",
           "action" -> "rename_org",
  ...
  def userHasOrgAccess(user: String, org: String) = DB.readOnly {
    implicit session =>
      info("status" -> "check",
           "action" -> "user_has_org_access",
           "user" -> user,
           "org" -> org)
      sql"""select exists(
          ...
          limit 1)""".map(_.boolean(1)).single.apply().get
  }
    ...
    Json.parse(c).asOpt[DataValidator] match {
      ...
      case None =>
        throw new IllegalArgumentException(
            s"invalid format for dataset validator: ${c}")
    }
  }

  def listDatasets(org: String, afterDate: Long = 32503680000L) = DB.readOnly {
    implicit session =>
      val datasets = sql"""
          select
          ...
          limit 100""".map { rs =>
        Dataset(rs.string("dataset_ident"),
  ...
  private def jodaFromEpoch(epoch: Double) =
    new DateTime((1000 * epoch).toLong).withZone(UTC)

  private val datasetDupeKeyMsgSnippet =
    """duplicate key value violates unique constraint "dataset_org_id_provided_id_key""""

  def createDataset(org: String, ds: Dataset) = DB.localTx {
    implicit session =>
      try {
        sql"""
        ...
        from org where org.provided_id = ${org}""".execute.apply()
      } catch {
        case e: PSQLException
            if e.getMessage.contains(datasetDupeKeyMsgSnippet) =>
          throw new DatasetIdAlreadyExists(s"$org/${ds.id}")
      }
  }

  def getDataset(org: String, id: String): Dataset = DB.readOnly {
    implicit session =>
      info("status" -> "starting",
           "action" -> "get_dataset",
           "org" -> org,
      ...
      group by dsid, ds_ident, dataset_type, dschema, d_created_at, d_updated_at""".map {
        rs =>
          val ds =
            Dataset(rs.string("ds_ident"),
                    datasetType(rs.string("dataset_type")),
                    datasetValidator(rs.string("dschema")),
                    createdAt = Some(jodaFromEpoch(rs.double("d_created_at"))),
                    updatedAt = Some(jodaFromEpoch(rs.double("d_updated_at"))),
      ...
      dsOpt match {
        case Some((dsid, ds)) => {
          ...
          select min, max, mean, stddev, p[1] median, p[2] p10, p[3] p90 from (
            ...
            from datum
            where dataset_id = ${dsid}::uuid and rvalue is not null) q1""".map {
              rs =>
                (rs.doubleOpt("min"),
                 rs.doubleOpt("max"),
                 rs.doubleOpt("mean"),
              ...
              summaryStats match {
                case Some(
                    (Some(mn),
                     ...
                     Some(p10),
                     Some(p90))) => {
                    ds.copy(summaryStats = Some(Dataset.RegressionSummaryStats(
                                mn, mx, mean, std, med, p10, p90)))
                  }
                case _ => ds
              }
            } else {
          ...
          case Some((uuid, dtypeStr)) => {
              ...
              updates.data foreach { dataBlock =>
                ...
                val batchInserts: Seq[Seq[Any]] = for {
                  ...
                  Seq(datumId.toString,
                      hex2int(datumIdShort),
                      data,
                      value,
                      uuid)
                }
  ...
  def deleteDataset(org: String, id: String) = {
    ...
    val delIds = DB.localTx { implicit session =>
      sql"""
          ...
          """.map(_.string("provided_id")).list.apply()
    }
    if (delIds.isEmpty) {
      throw new NotFoundInDb()
    } else if (delIds != List(id)) {
      val msg =
        s"unexpected datasets deleted matching $org/$id: ${delIds.mkString(", ")}"
      throw new UnexpectedResultException(msg)
    } else {
      info("status" -> "success",
           "action" -> "delete_dataset",
  ...
  def listData(org: String,
               ...
               after: Option[String] = None,
               max: Option[Int] = None,
               withVal: Boolean = false) = DB.readOnly { implicit session =>
    info("status" -> "starting",
         "action" -> "search_data",
         "org" -> org,
    ...
    val datasetDetails = sql"""
        ...
        group by d_id""".map { rs =>
      (rs.string("d_id"), rs.int("total_count"))
    }.single.apply()
    ...
    val datumQ: Seq[Datum] = sql"""
      ...
      val value: Either[Double, String] = (rvalue, cvalue) match {
        ...
        case _ =>
          throw new UnexpectedResultException(
              s"Unprocessed value for datum ${org}/${datasetId}/${ident}: ${(cvalue, rvalue)}")
      }
  ...
  def listExperiments(org: String): ResultSet[Experiment] = DB.readOnly {
    implicit session =>
      ...
      val experiments = sql"""
        ...
        where org.provided_id = ${org}""".map { rs =>
        Experiment(rs.string("exp_id"), Some(rs.string("dataset_id")))
      }.list.apply()
  ...
  def createExperiment(
      org: String, datasetId: String, experiment: Experiment) = DB.localTx {
    implicit session =>
      info("status" -> "starting",
      ...
      val datasetUuidOpt = sql"""
        ...
        from dataset
          inner join org on dataset.org_id = org.org_id
        where dataset.provided_id = ${datasetId}
          and org.provided_id = ${org}""".map(_.string("did")).single.apply()
      ...
      datasetUuidOpt match {
        case Some(datasetUuid) => {
            try {
              ...
              values (${experiment.id}, ${datasetUuid}::uuid)""".update.apply()
              info("status" -> "success",
                   "action" -> "create_experiment",
                   "org" -> org,
                   "dataset" -> datasetId,
              ...
              case e: PSQLException
                  if e.getMessage.contains(experimentDupeKeyMsgSnippet) =>
                throw new ExperimentIdAlreadyExists(
                    s"$org/$datasetId/${experiment.id}")
            }
          }
        case None => {
            throw new NotFoundInDb
  ...
  def getExperiment(org: String, datasetId: String, experimentId: String) =
    DB.readOnly { implicit session =>
      info("status" -> "starting",
      ...
      val experimentOpt = sql"""
        ...
        where experiment.provided_id = ${experimentId}
          and org.provided_id = ${org}
          and dataset.provided_id = ${datasetId}""".map { rs =>
        val experiment =
          Experiment(rs.string("exp_id"),
                     datasetId = Some(rs.string("data_id")),
                     createdAt =
                       Some(jodaFromEpoch(rs.double("e_created_at"))),
                     updatedAt =
                       Some(jodaFromEpoch(rs.double("e_updated_at"))),
                     datasetType =
                       Some(datasetType(rs.string("dataset_type"))))
      ...
      experimentOpt match {
        case Some((experimentUuid, datasetUuid, experiment)) => {
            ...
            val sizeOpt =
              sql"""select count(1) from prediction where experiment_id = ${experimentUuid}::uuid"""
                .map(_.int(1))
                .single
                .apply()

            val dsSizeOpt =
              sql"""select count(1) from datum where dataset_id = ${datasetUuid}::uuid"""
                .map(_.int(1))
                .single
                .apply()
            ...
            experiment.datasetType match {
              case Some(Dataset.ClassificationType) => {
                  val classes =
                    sql"""select distinct cvalue from datum where dataset_id = ${datasetUuid}::uuid"""
                      .map(_.string("cvalue"))
                      .list
                      .apply()
                ...
                group by gold_cls, pred_cls""".map { rs =>
                    (rs.string("gold_cls"),
                     rs.string("pred_cls"),
                     rs.int("num"))
                  }.list.apply()

                  experiment.copy(size = sizeOpt,
                                  datasetSize = dsSizeOpt,
                                  classificationResults =
                                    Some(Experiment.ClassificationResults(
                                            classes, classCounts)))
                }
                ...
                select
                  ...
                  select abs(diff) abs_diff, diff * diff diffsq from (
                    select datum.rvalue - prediction.rvalue diff
                      ...
                      where experiment_id = ${experimentUuid}::uuid) b) c) d""".map {
                    rs =>
                      (
                          rs.doubleOpt("rmse"),
                        ...
                        case (Some(rmse),
                              ...
                              Some(p10),
                              Some(p90)) => {
                            Some(Experiment.RegressionResults(
                                    rmse, aae, mae, p10, p90))
                          }
                        case _ => None
                      }
              ...
              case _ =>
                experiment.copy(size = sizeOpt, datasetSize = dsSizeOpt)
            }
          }
        case None => {
            throw new NotFoundInDb
  ...
  def updateExperiment(org: String,
                       datasetId: String,
                       experimentId: String,
                       updates: Experiment.Update) = DB.localTx {
    implicit session =>
      try {
        info("status" -> "starging",
        ...
        idOpt match {
          case Some((experimentUuid, datasetUuid, dtypeStr)) => {
              ...
              updates.data foreach { dataBlock =>
                ...
                val datumUuids = {
                  val datumIdents =
                    Buffer(dataBlock.map(_.datumId).map(hex2int): _*)
                  sql"""
                  select to_hex(ident) datum_ident, datum_id::text datum_uuid
                  from datum
                  where dataset_id = ${datasetUuid}::uuid and ident in (${datumIdents})""".map {
                    rs =>
                      rs.string("datum_ident") -> rs.string("datum_uuid")
                  }.list.apply()
                }
                ...
                val batchInserts: Seq[Seq[Any]] = for {
                  ...
                  Seq(newUuid.toString,
                      hex2int(newIdent),
                      datumUuid.toString,
                      experimentUuid.toString,
                      prediction.valueAsAny)
                }
  ...
  def deleteExperiment(org: String, datasetId: String, experimentId: String) = {
    ...
    if (delIds.isEmpty) {
      ...
      val msg =
        s"unexpected experiments deleted matching $org/$datasetId/$experimentId: ${delIds.mkString(", ")}"
      throw new UnexpectedResultException(msg)
    } else {
      info("status" -> "success",
