...
object ExperimentController extends Controller with JsonLogging {
    ...
    HasOrgAccess(user, org) {
      Action { implicit request =>
        try {
          ...
          render {
            case AcceptsText() =>
              Ok(Format.experimentRStoString(results) + "\n")
            case Accepts.Html() => Ok(views.html.ExperimentApi.search(results))
            case Accepts.Json() => Ok(Json.toJson(results))
          }
        } catch {
    ...
    HasOrgAccess(user, org) {
      Action(parse.json) { implicit request =>
        request.body
          .validate[Experiment]
          .fold(
              errors => BadRequest(JsError.toFlatJson(errors)), { exp =>
                try {
                  dbase.createExperiment(org, datasetId, exp)
                  infoU("status" -> "success",
                        "action" -> "create",
                        "experiment" -> s"$org/$datasetId/${exp.id}")
                  val saved = db.impl.getExperiment(org, datasetId, exp.id)
                  render {
                    case AcceptsText() =>
                      Ok(Format.experimentToString(saved) + "\n")
                    case Accepts.Html() =>
                      Ok(views.html.ExperimentApi.details(saved))
                  ...
                  case _: ExperimentIdAlreadyExists => {
                      val msg =
                        s"Experiment ID already exists: $org/$datasetId/${exp.id}"
                      infoU("status" -> "error", "message" -> msg)
                      BadRequest(msg + "\n")
                    }
                }
  ...
  def details(org: String, datasetId: String, id: String) = Authenticated {
    implicit user =>
      HasOrgAccess(user, org) {
        Action { implicit request =>
          try {
  ...
  def update(org: String, datasetId: String, id: String) = Authenticated {
    implicit user =>
      HasOrgAccess(user, org) {
        Action(parse.json) { implicit request =>
                ...
                errors => BadRequest(JsError.toFlatJson(errors)), {
                  expUpdate =>
                    try {
                      ...
                      case _: NotFoundInDb => {
                          val msg =
                            s"Experiment not found: $org/$datasetId/$id"
                          render {
                            case AcceptsText() => NotFound(msg + "\n")
                            case Accepts.Json() =>
  ...
  def delete(org: String, datasetId: String, experimentId: String) =
    Authenticated { implicit user =>
      HasOrgAccess(user, org) {
        Action { implicit request =>
          ...
          try {
            val savedBeforeDelete =
              dbase.getExperiment(org, datasetId, experimentId)
            ...
            render {
              case AcceptsText() =>
                Ok(Format.experimentToString(savedBeforeDelete) + "\n")
              case Accepts.Html() =>
                Ok(views.html.ExperimentApi.details(savedBeforeDelete))
              case Accepts.Json() => Ok(Json.toJson(savedBeforeDelete))
            }
          } catch {
            case _: NotFoundInDb => {
                val msg =
                  s"Experiment not found: $org/$datasetId/$experimentId"
                render {
                  case AcceptsText() => NotFound(msg)
                  case Accepts.Json() => NotFound(Json.obj("message" -> msg))
                }
