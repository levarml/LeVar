  ...
  def search(org: String) = Authenticated { implicit user =>
    HasOrgAccess(user, org) {
      Action { implicit request =>
        ...
        infoU("status" -> "response",
              "action" -> "search_for_datasets",
              "result" -> s"${results.size} results")
        render {
      ...
      Action(parse.json) { implicit request =>
        ...
        try {
          request.body
            .validate[Dataset]
            .fold(
                errors => BadRequest(JsError.toFlatJson(errors)), { ds =>
                  try {
                    ...
                    render {
                      case AcceptsText() =>
                        Ok(Format.datasetToString(saved) + "\n")
                      case Accepts.Html() =>
                        Ok(views.html.DatasetApi.details(saved))
        ...
        try {
          ...
          case _: NotFoundInDb => {
              ...
              render {
                ...
                case Accepts.Html() =>
                  NotFound(views.html.DatasetApi.notfound(id))
    ...
    HasOrgAccess(user, org) {
      Action(parse.json) { implicit request =>
        ...
        request.body
          ...
          .fold(
              errors => BadRequest(JsError.toFlatJson(errors)), { dsUpdate =>
                try {
                  ...
                  case _: DatasetIdAlreadyExists => {
                      val msg =
                        s"dataset ID ${dsUpdate.id.getOrElse("<?>")} already exists"
                      infoU("status" -> "error",
                            "action" -> "update",
                            "message" -> msg)
                      render {
    ...
    HasOrgAccess(user, org) {
      Action { implicit request =>
        ...
        try {
          ...
          render {
            case AcceptsText() =>
              Ok(Format.datasetToString(savedBeforeDelete) + "\n")
            case Accepts.Html() =>
              Ok(views.html.DatasetApi.details(savedBeforeDelete))
