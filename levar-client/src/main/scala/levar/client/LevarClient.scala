...
object LevarClient {
  ...
  val knownHttpErrs = Map(400 -> "Bad Request",
                          401 -> "Unauthorized",
...
class LevarClient(val config: ClientConfig) {
  ...
  private def post[A](
      path: String, body: A)(implicit f: Writes[A]): Future[Unit] =
    client
      .url(config.url + path)
  ...
  private def get[A](path: String, query: (String, String)*)(
      implicit f: Reads[A]): Future[A] = {
    client
      .url(config.url + path)
      .withAuth(config.username, config.password, WSAuthScheme.BASIC)
      ...
      .map { response =>
        if (response.status == 200) response.json.as[A]
          ...
          val msg =
            if (response.statusText.nonEmpty) {
              response.statusText
            } else {
  ...
  def uploadDatasetData(ds: String,
                        data: Seq[Datum],
                        orgOpt: Option[String] = None): Future[Unit] = {
    val org = orgOpt.getOrElse(config.org)
  ...
  def renameDataset(org: String, id: String, newId: String): Future[Unit] = {
    post[Dataset.Update](
        s"/api/$org/$id/update", Dataset.Update(id = Some(newId)))
  }

  def fetchData(org: String,
                datasetId: String,
                includeGold: Boolean): Future[ResultSet[Datum]] = {
  ...
  def fetchData(org: String,
                datasetId: String,
                includeGold: Boolean,
                after: String): Future[ResultSet[Datum]] = {
    val g = if (includeGold) "1" else "0"
    get[ResultSet[Datum]](
        s"/api/$org/$datasetId/data?gold=${g}&after=${encode(after, "utf-8")}")
  }

  def searchExperiments(
      org: String, datasetId: String): Future[ResultSet[Experiment]] = {
    get[ResultSet[Experiment]](s"/api/$org/$datasetId/experiments") flatMap {
      rs =>
        val futures = rs.items.map(ds => getExperiment(org, datasetId, ds.id))
        Future.sequence(futures).map { datasets =>
          rs.copy(items = datasets)
        }
    }
  }
  ...
  def createExperiment(
      org: String, datasetId: String, experiment: Experiment): Future[Unit] = {
    post[Experiment](s"/api/$org/$datasetId/new_experiment", experiment)
  }

  def uploadExperimentData(org: String,
                           datasetId: String,
                           experimentId: String,
                           data: Seq[Prediction]): Future[Unit] = {
    val upd = Experiment.Update(data = Some(data))
    post[Experiment.Update](s"/api/$org/$datasetId/$experimentId/update", upd)
  }

  def getExperiment(org: String,
                    datasetId: String,
                    experimentId: String): Future[Experiment] = {
    get[Experiment](s"/api/$org/$datasetId/$experimentId")
  }

  def deleteExperiment(
      org: String, datasetId: String, experimentId: String): Future[Unit] = {
