...
class JsonSpec extends FlatSpec {
  ...
  "Comment JSON serialization" should "create basic JSON" in {
    assertWrites("""{"username":"john", "comment": "Hello world"}""",
    ...
    assertWrites("""|{
         ...
         |""".stripMargin,
                 Comment("john",
                         "Hello world!",
                         path =
                           Some("/api/foo/experiment/1234/comments/abcdef")))
  }
  ...
  it should "handle subjects" in {
    assertWrites(
        ...
        Comment(
            ...
            "Hello world!",
            subject = Some(
                ("experiment", Json.parse("""{"id": "silly-experiment"}""")))))
  }
  ...
  "Comment JSON deserialization" should "parse basic JSON" in {
    assertReads[Comment](
        Comment("john", "Yo"), """{"username": "john", "comment": "Yo"}""")
  }
  ...
  "Datum JSON serialization" should "create basic JSON" in {
    assertWrites(
        ...
        Datum(Json.obj(
                "text" -> "Hello world!", "score" -> 123.4, "count" -> 987))
    )
  }
  ...
  it should "handle real values" in {
    assertWrites(
        ...
        Datum(Json.obj("text" -> "Hello world!",
                       "score" -> 123.4,
                       "count" -> 987),
    ...
    assertWrites("""|{
         ...
         |""".stripMargin,
                 Datum(Json.obj("text" -> "Hello world!",
                                ...
                                "count" -> 987),
                       createdAt =
                         Some(new DateTime(2005, 3, 26, 12, 0, 0, 0, UTC))))
  }
  ...
  it should "handle comments" in {
    assertWrites(
        ...
        Datum(Json.obj(
              ...
              id = Some("hello"),
              comments =
                Some(ResultSet(Seq(Comment("john", "this is cool!"))))))
  }
  ...
  it should "handle created at" in {
    assertReads(Datum(Json.obj("text" -> "Hello world!",
                               "score" -> 123.4,
                               "count" -> 987),
                      createdAt =
                        Some(new DateTime(2005, 3, 26, 12, 0, 0, 0, UTC))),
  ...
  "Result set JSON serialization" should "create basic JSON" in {
    assertWrites(
        """{"items": ["big", "data"]}""", ResultSet(Seq("big", "data")))
  }

  it should "handle paths" in {
    assertWrites(
        """{"items": ["big", "data"], "path": "/api/foo/dataset/dataset-1/labels"}""",
        ResultSet(Seq("big", "data"),
                  Some("/api/foo/dataset/dataset-1/labels")))
  }
  ...
  it should "handle next_path" in {
    assertWrites(
        ...
        ResultSet(Seq("big", "data"),
                  total = Some(1001),
                  nextPath =
                    Some("/api/foo/dataset/dataset-1/labels?after=123412345")))
  }

  "Result set JSON deserialization" should "create basic JSON" in {
    assertReads(
        ResultSet(Seq("big", "data")), """{"items": ["big", "data"]}""")
  }
  ...
  it should "handle total" in {
    assertReads(ResultSet(Seq("big", "data"), total = Some(1001)),
                """|{
         |  "items": ["big", "data"],
         |  "total": 1001
         |}
  ...
  "Experiment JSON serialization" should "create basic JSON" in {
    assertWrites("""{"id": "hello-world"}""", Experiment("hello-world"))
  }

  it should "handle datasetId" in {
    assertWrites("""{"id": "hello-world", "dataset_id": "foo"}""",
                 Experiment("hello-world", datasetId = Some("foo")))
  }

  it should "handle name" in {
    assertWrites("""{"id": "hello-world", "name": "Hello world!"}""",
  ...
  it should "handle size" in {
    assertWrites("""{"id": "hello-world", "size": 1000}""",
                 Experiment("hello-world", size = Some(1000)))
  }

  it should "handle labels" in {
    assertWrites("""{"id": "hello-world", "labels": ["big", "data"]}""",
  ...
  it should "handle comments" in {
    assertWrites(
        ...
        Experiment(
            "hello-world",
            comments = Some(
                ResultSet(Seq(Comment("john", "yo"), Comment("mary", "lo"))))))
  }

  it should "handle classification results" in {
    val results = Experiment.ClassificationResults(
        Seq("yes", "no"), Seq(("yes", "yes", 1), ("no", "yes", 2)))
    val experimentExp =
      Experiment("hello-world", classificationResults = Some(results))
    val jsonString = Json.toJson(experimentExp).toString
    val experimentAct = Json.parse(jsonString).as[Experiment]
    assert(
        results.classes.toSet == experimentAct.classificationResults.get.classes.toSet)
    assert(
        results.classCounts.toSet == experimentAct.classificationResults.get.classCounts.toSet)
  }

  "Experiment JSON deserialization" should "create basic JSON" in {
    assertReads(Experiment("hello-world"), """{"id": "hello-world"}""")
  }

  it should "handle datasetIds" in {
    assertReads(Experiment("hello-world", datasetId = Some("foo")),
                """{"id": "hello-world", "dataset_id": "foo"}""")
  }

  it should "handle name" in {
    assertReads(Experiment("hello-world", name = Some("Hello world!")),
  ...
  it should "handle size" in {
    assertReads(Experiment("hello-world", size = Some(1000)),
                """{"id": "hello-world", "size": 1000}""")
  }

  it should "handle labels" in {
    assertReads(Experiment("hello-world", labels = Some(Seq("big", "data"))),
  ...
  "Prediction JSON serialization" should "handle categorical predictions" in {
    assertWrites("""{"data_id": "123456","value": "yes"}""",
                 Prediction("123456", Right("yes")))
  }

  it should "handle real valued predictions" in {
    assertWrites("""{"data_id": "123456","value": 0.456}""",
                 Prediction("123456", Left(0.456)))
  }
  ...
  it should "handle inputs" in {
    assertWrites(
        ...
        Prediction("123456",
                   Left(0.456),
                   datum = Some(Datum(Json.obj("text" -> "Yo")))))
  }

  it should "handle score" in {
    assertWrites("""{"data_id": "123456", "value": 0.234, "score": 0.998}""",
  ...
  "Prediction JSON deserialization" should "handle categorical predictions" in {
    assertReads(Prediction("123456", Right("yes")),
                """{"data_id": "123456","value": "yes"}""")
  }

  it should "handle real valued predictions" in {
    assertReads(Prediction("123456", Left(0.456)),
                """{"data_id": "123456","value": 0.456}""")
  }
  ...
  it should "handle score" in {
    assertReads(Prediction("123456", Left(0.234), score = Some(0.998)),
