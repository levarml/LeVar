...
package object json {
  ...
  implicit val JodaDateFormat = new Format[DateTime] {
    ...
    private val reader =
      Reads.StringReads.map(tsparser.parseDateTime(_).withZone(UTC))
  ...
  implicit def ResultSetFormat[A](
      implicit fmt: Format[A]): Format[ResultSet[A]] =
    (
                      ...
                      case JsObject(fieldData) => {
                          fieldData.find(_._1 == "type").map(_._2) match {
                            ...
                            case Some(JsString("number")) =>
                              fields += ((key, NumberField))
                            case _ =>
                              errs +=
                                s"unrecognized field type for $key, ${dtype.toString}"
                          }
                        }
                      case _ =>
                        errs += ("need to include \"type\" field for " + key)
                    }
                }
                if (errs.isEmpty) {
  ...
  implicit lazy val RegressionSummaryStats: Format[
  ...
  )(Dataset.RegressionSummaryStats.apply _,
    unlift(Dataset.RegressionSummaryStats.unapply))
  ...
  implicit lazy val ExperimentFormat: Format[Experiment] = (
      ...
      (__ \ "classification_results")
        .formatNullable[Experiment.ClassificationResults] and
  ...
  implicit def Tuple3Format[A, B, C](implicit fmtA: Format[A],
                                     fmtB: Format[B],
                                     fmtC: Format[C]): Format[(A, B, C)] = {
    val reads = Reads[(A, B, C)] { js =>
      val JsArray(ls) = js
      JsSuccess((ls(0).as[A], ls(1).as[B], ls(2).as[C]))
    }
    val writes = Writes[(A, B, C)]((x: (A, B, C)) => {
      JsArray(Seq(Json.toJson(x._1), Json.toJson(x._2), Json.toJson(x._3)))
    })
    Format(reads, writes)
  }

  implicit lazy val ExperimentClassificationResults: Format[
      Experiment.ClassificationResults] = (
      (__ \ "labels").format[Seq[String]] and
      (__ \ "class_counts").format[Seq[(String, String, Int)]]
  )(Experiment.ClassificationResults.apply _,
    unlift(Experiment.ClassificationResults.unapply))

  implicit lazy val ExperimentRegressionResults: Format[
  ...
  )(Experiment.RegressionResults.apply _,
    unlift(Experiment.RegressionResults.unapply))
}
