    ...
    try {
      ...
      val args = new ScallopConf(argv) {
        ...
        val configCmd = new Subcommand("config") {
          ...
          val username = opt[String]("username", descr = "Your username")

          val password = opt[String]("password", descr = "Your password")
        ...
        val listCmd = new Subcommand("list") {
          ...
          val datasetsCmd = new Subcommand("datasets") {
            ...
            val namesOnly = toggle(name = "namesonly",
                                   default = Some(false),
                                   descrYes = "Only display dataset names")
            val org =
              trailArg[String](required = false, descr = "Your organization")
          }
          ...
          val experimentsCmd = new Subcommand("experiments") {
            ...
            val dataset = trailArg[String](
                "dataset",
                required = false,
                descr = "List experiments for this dataset")
          }
        }
        ...
        val uploadCmd = new Subcommand("upload") {
          ...
          val datasetCmd = new Subcommand("dataset") {
            ...
            val name = opt[String](
                "name",
                descr = "Name for the dataset (default = the filename)")
            val org = opt[String](
                "org", descr = "Upload dataset to a specific organization")
            val file =
              trailArg[String](required = true, descr = "TSV file for upload")
          }
          ...
          val experimentCmd = new Subcommand("experiment") {
            ...
            val dataset = trailArg[String](
                required = true,
                descr = "Dataset associated with the experiment")
            val file = trailArg[String](
                required = true,
                descr =
                  "TSV file for upload; must include id and score/class columns")
          }
        }
        ...
        val viewCmd = new Subcommand("view") {
          ...
          val datasetCmd = new Subcommand("dataset") {
            ...
            val dataset = trailArg[String](
                required = true,
                descr = "Dataset to view (by ID or org/id pattern)")
          }

          val experimentCmd = new Subcommand("experiment") {
            banner(" View a summary of experiment results")
            footer("")
            val experiment = trailArg[String](
                required = true,
                descr =
                  "Experiment to view -- specify like DATASET/EXPERIMENT")
          }
        }
        ...
        val renameCmd = new Subcommand("rename") {
          ...
          val datasetCmd = new Subcommand("dataset") {
            ...
            val from =
              trailArg[String](required = true, descr = "Dataset to rename")
            val to = trailArg[String](
                required = true, descr = "New name for the dataset")
          }
          ...
          val experimentCmd = new Subcommand("experiment") {
            ...
            val from = trailArg[String](
                required = true,
                descr =
                  "Experiment to rename -- specify like DATASET/EXPERIMENT")
            val to = trailArg[String](
                required = true,
                descr = "New name for the experiment (omit dataset path)")
          }
        }
        ...
        val deleteCmd = new Subcommand("delete") {
          ...
          val datasetCmd = new Subcommand("dataset") {
            ...
            footer("")
            val dataset =
              trailArg[String](required = true, descr = "Dataset to delete")
          }
          ...
          val experimentCmd = new Subcommand("experiment") {
            ...
            val experiment = trailArg[String](
                required = true,
                descr =
                  "Experiment to delete -- specify like DATASET/EXPERIMENT")
          }
        }
        ...
        val downloadCmd = new Subcommand("download") {
          ...
          val datasetCmd = new Subcommand("dataset") {
            banner(
                " Download a dataset from the database. By default does *not* include gold labels/values\n\n options:")
            footer("")

            val output =
              opt[String]("output", descr = "File to save dataset to")
            val gold = toggle(name = "gold",
                              default = Some(false),
                              descrYes =
                                "Include gold labels/values in output")
            val dataset =
              trailArg[String](required = true, descr = "Dataset to view")
          }
        }
        ...
        val previewCmd = new Subcommand("preview") {
          val datasetCmd = new Subcommand("dataset") {
            banner(
                " Ingest a TSV dataset and see a high level summary.\n\n options:")
      ...
      args.subcommands match {
            ...
            ClientConfigIo.loadClientConfig match {
              case Some(cfg) => {
                  ...
                  if (!setAnything) {
                    ...
                    val newUser =
                      readLine(s"Username [${cfg.username}]: ").trim
                    if (newUser.nonEmpty) {
                    ...
                    while (!haveValidOrg) {
                      val newOrg =
                        readLine(s"Organization [${cfg.org}]: ").trim
                      if (newOrg.nonEmpty) {
              ...
              case None => {
                  ...
                  while (password.isEmpty) {
                    ...
                    print("Verify password: ")
                    val passwordVerify =
                      new String(standardIn.readPassword).trim
        ...
        case List(args.listCmd, args.listCmd.datasetsCmd) => {
            ...
            val org =
              args.listCmd.datasetsCmd.org.get.getOrElse(client.config.org)
            try {
              val datasetRS =
                Await.result(client.searchDatasets(org), waitTime seconds)
            ...
            try {
              ...
              if (!(fileName.endsWith(".tsv") || fileName.endsWith(".txt"))) {
                throw new RuntimeException(
                    "Expected to see a TSV file (.tsv or .txt)")
              }
              ...
              try {
                ...
                Await.result(
                    client.createDataset(dataset, org), waitTime seconds)
                for (data <- tabular.data.grouped(250)) {
                  val d = data.toSeq
                  Await.result(client.uploadDatasetData(dataset.id, d, org),
                               waitTime seconds)
                }
                val saved =
                  Await.result(client.getDataset(org, name), waitTime seconds)
              ...
              case _: MissingClientConfig => {
                  Console.err.println(
                      "Missing cofig -- run `levar config` to set up")
                  sys.exit(1)
                }
            }
          }
        ...
        case List(args.viewCmd, args.viewCmd.datasetCmd) => {
            ...
            try {
              val dataset = Await.result(
                  client.getDataset(Some(org), datasetId), waitTime seconds)
            ...
            try {
              ...
              dataset.dtype match {
                ...
                case Dataset.RegressionType => {
                    ...
                    val stats = Dataset.RegressionSummaryStats(
                        dmin, dmax, dmean, dstddev, dmedian, p10, p90)
        ...
        case List(args.deleteCmd, args.deleteCmd.datasetCmd) => {
            ...
            readLine("> ").trim match {
              case abPatt(orgVerify, datasetIdVerify)
                  if orgVerify == org && datasetIdVerify == datasetId => {
                  Await.result(
                      client.deleteDataset(org, datasetId), waitTime seconds)
                  println("Deleted")
                }
              case _ => {
        ...
        case List(args.downloadCmd, args.downloadCmd.datasetCmd) => {
            ...
            try {
              ...
              val dataset = Await.result(
                  client.getDataset(org, datasetId), waitTime seconds)

              val cols =
                Seq("id") ++ dataset.columns ++ {
                  if (useGold) Seq(dataset.dtype.colname) else Seq.empty
                }

              val stream: PrintStream =
                args.downloadCmd.datasetCmd.output.get match {
              ...
              try {
                ...
                var rs = Await.result(
                    client.fetchData(org, datasetId, useGold),
                    waitTime seconds)
                while (rs.nonEmpty) {
                  for (datum <- rs) {
                    stream.println(
                        datum.dataByCol(cols).map(_.toString).mkString("\t"))
                  }
                  rs.lastOption.flatMap(_.id) foreach { id =>
                    rs = Await.result(
                        client.fetchData(org, datasetId, useGold, id),
                        waitTime seconds)
                  }
                }
              } finally {
              ...
              val experimentRS = dsNameOpt match {
                case Some(dsName) => {
                    val (datasetOrg, datasetId) = dsName match {
                      case abPatt(datasetOrg, datasetId) =>
                        (datasetOrg, datasetId)
                      case aPatt(datasetId) => (client.config.org, datasetId)
                      case _ => {
                          Console.err.println(s"Invalid dataset name: $dsName")
                          sys.exit(1)
                    ...
                    Await.result(
                        client.searchExperiments(datasetOrg, datasetId),
                        waitTime seconds)
                  }
                case None => {
        ...
        case List(args.uploadCmd, args.uploadCmd.experimentCmd) => {
            ...
            val name =
              args.uploadCmd.experimentCmd.name.get.getOrElse(fileName)
            ...
            try {
              ...
              Await.result(client.createExperiment(org, datasetId, experiment),
                           waitTime seconds)
              for (data <- tabular.data.grouped(1000)) {
                val d = data.toSeq
                Await.result(client.uploadExperimentData(
                                 org, datasetId, experiment.id, d),
                             waitTime seconds)
              }
              val saved = Await.result(
                  client.getExperiment(org, datasetId, name), waitTime seconds)
        ...
        case List(args.renameCmd, args.renameCmd.datasetCmd) => {
            ...
            Await.result(client.renameDataset(org, datasetId, newDatasetId),
                         waitTime seconds)
        ...
        case List(args.deleteCmd, args.deleteCmd.experimentCmd) => {
            ...
            val (org, datasetId, experimentId) = expName match {
              case abcPatt(orgId, datasetId, experimentId) =>
                (orgId, datasetId, experimentId)
              case abPatt(datasetId, experimentId) =>
                (client.config.org, datasetId, experimentId)
              case _ => {
                  Console.err.println(s"Invalid experiment name: $expName")
                  Console.err.println(
                      "Please specify as DATASET/EXPERIMENT or ORG/DATASET/EXPERIMENT")
                  sys.exit(1)
                }
            }
            ...
            if (readLine("> ").trim == experimentId) {
              Await.result(
                  client.deleteExperiment(org, datasetId, experimentId),
                  waitTime seconds)
              println("Deleted")
            } else {
              Console.err.println("Org + dataset do not match")
        ...
        case List(args.viewCmd, args.viewCmd.experimentCmd) => {
            ...
            val experiment = Await.result(
                client.getExperiment(org, datasetId, experimentId),
                waitTime seconds)
        ...
        case _ =>
          println("You did not supply an argument -- try 'levar-cli datasets'")
      }
    } catch {
